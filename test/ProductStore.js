...
describe("ProductStore", function () {
    async function deployOwnedFixture() {
        ...
        const productStore = await ProductStore.connect(otherAccount).deploy(await productIdentification.getAddress(), await productDeposit.getAddress());
    ...
    it("Product price per unit should be greater than zero", async function () {
        ...
        await expect(productStore.connect(otherAccount).setProductPricePerUnit(0)).to.be.revertedWith("ProductStore: product price per unit must be greater than zero");
    });

    it("Non-owner producer should not be able to set product price per unit", async function () {
        const { productIdentification, productDeposit, productStore, owner, otherAccount } = await loadFixture(deployOwnedFixture);

        await expect(productStore.connect(owner).setProductPricePerUnit(100)).to.be.revertedWithCustomError(productStore, "OwnableUnauthorizedAccount");
    });
    ...
    it("Non-owner producer should not be able to transfer product to store", async function () {
        ...
        await expect(productStore.connect(owner).transferProductToStore(1, 40)).to.be.revertedWithCustomError(productStore, "OwnableUnauthorizedAccount");
    });

    it("Should revert when transferring product to store with insufficient stock", async function () {
        const { productIdentification, productDeposit, productStore, owner, otherAccount } = await loadFixture(deployOwnedFixture);

        await expect(productStore.connect(otherAccount).transferProductToStore(1, 40)).to.be.revertedWith("ProductStore: product is not available in deposit");
    });
    ...
    it("Should revert when buying product with insufficient stock", async function () {
        ...
        await expect(productStore.connect(otherAccount1).buyProduct(1, { value: 20 })).to.be.revertedWith("ProductStore: product is not available");
    });
    ...
    it("Should revert when buying product with insufficient payment", async function () {
        ...
        await expect(productStore.connect(otherAccount1).buyProduct(1, { value: 19 })).to.be.revertedWith("ProductStore: insufficient payment");
    });

    it("Transfer amount should be greater than zero", async function () {
        const { productIdentification, productDeposit, productStore, owner, otherAccount, otherAccount1 } = await loadFixture(deployOwnedFixture);

        await expect(productStore.connect(otherAccount).transferProductToStore(1, 0)).to.be.revertedWith("ProductStore: transfer amount must be greater than zero");
    });
});